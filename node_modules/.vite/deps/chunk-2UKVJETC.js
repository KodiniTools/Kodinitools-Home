  ...
  while (position < html.length && !foundBodyStart) {
    ...
    if (tagId === TAG_HTML) {
      ...
      if (tagId === TAG_TITLE) {
        ...
        lastCopyPosition = position;
        (input.script || (input.script = [])).push(scriptAttrs);
        continue;
        ...
        lastCopyPosition = position;
        (input.style || (input.style = [])).push(styleAttrs);
        continue;
      } else if (tagId === TAG_META) {
        (input.meta || (input.meta = [])).push(attributes);
        position = tagEnd;
        lastCopyPosition = position;
        continue;
      } else if (tagId === TAG_LINK) {
        (input.link || (input.link = [])).push(attributes);
        position = tagEnd;
        lastCopyPosition = position;
        continue;
...
export {
  ...
  parseAttributes,
  parseHtmlForIndexes,
  parseHtmlForUnheadExtraction,
  applyHeadToHtml
};
//# sourceMappingURL=chunk-2UKVJETC.js.map
