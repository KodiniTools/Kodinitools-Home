...
- `server/admin/public/` — eigenständige Vanilla-JS-App (kein Build-Schritt), vom Node-Dienst ausgeliefert:
  ...
  - **Tool-Karten** (DE/EN): Rahmen (Farbe/Transparenz/Breite/Linienart/Radius), Hintergrund (Farbe/Transparenz/Verlauf) und Hover-Farben der Tool-Karten – Standard für alle Karten plus Einzel-Designs je Karte, getrennt Hell/Dunkel. Layout dreispaltig: Seitenleiste **Hell** links und **Dunkel** rechts (je Modus Rahmen, Hintergrund, Hintergrundbild, Hover, Text-Farben; Felder tragen `data-mode`, IDs je Modus eindeutig; „Nach Dunkel/Hell kopieren“ je Leiste), in der Mitte die Sticky-Live-Vorschau beider Modi, Texte, Typografie und „Design übertragen“, darunter die Übersicht aller Karten mit eigenem Hell/Dunkel-Schalter; unter 1500 px stapeln sich die Spalten (`.tc-layout`, `.app-wide` via `renderMain`). **Karten ausblenden**: je gewählter Karte „Von der Seite ausblenden“ / „Wieder einblenden“ (Sticky-Vorschau) und die Liste „Ausgeblendete Karten“ (Einblenden je Karte / alle) – gilt für DE + EN, Speicherort `media.site.hiddenCards` (Schlüssel `sektion.key`, Server validiert Form/Doppelte/Anzahl); die Startseiten filtern die Kartenlisten (`getHiddenCards()`), das Favoriten-Skript blendet gespeicherte Favoriten über `data-hidden-keys` am `#favoritesGrid` aus; Design und Texte der Karte bleiben erhalten (jederzeit umkehrbar). „Design übertragen": Design einer Karte per Checkbox-Liste auf ausgewählte Karten anwenden, als neuen Standard übernehmen oder Karten auf den Standard zurücksetzen. Speicherort `media.<lang>.toolCards`; die Seite rendert daraus `--tc-*`-CSS-Variablen (`content.ts` → `getToolCardsCss`, `tool-cards.css`). **Texte der Karte** (Titel – mit Enter zweizeilig, Admin normalisiert auf max. 2 nicht-leere Zeilen mit `\n`, Seite und Admin-Vorschau rendern `.tool-card-title`/`.tc-title` mit `white-space: pre-line`, Listen/Auswahl zeigen den Titel einzeilig als „Zeile 1 / Zeile 2“; die Kartenhöhe ist ohnehin auf zwei Titelzeilen ausgelegt –, Badge, Beschreibung/Popup, Link) sind im selben Tab je gewählter Karte editierbar und landen als Text-Overrides der Sprache (`overrides.<lang>.json`, Pfad `<sektion>.<key>.<feld>`, leer = Standard aus der Sprachdatei; Badge leer = kein Badge; Link nur `https://…`, `/pfad` oder `#`); beim Standard die globale „Öffnen"-Beschriftung (`tool.open`). **Text-Design**: je Modus Text-Farben (Titel, Badge-Text, Badge-Hintergrund + Transparenz, „Öffnen"-Link, Popup-Text/-Hintergrund; An-Schalter, aus = Seitenstandard) in `toolCards.default|cards[id].light|dark.*Color`, und geteilte **Typografie** (`…text`: Schriftart Titel/Text aus `/fonts`, Größe, Gewicht, Buchstabenabstand, Schreibweise, Ausrichtung `align` links/zentriert/rechts für Icon, Badge, Titel und Popup-Text; 0/'' = Standard). **Elemente ein-/ausblenden** (Mitte, Sektion „👁️ Elemente ein-/ausblenden“, gilt für Hell + Dunkel): je Design lassen sich Icon, Badge, Titel, Favoriten-Knopf, „Öffnen"-Link und Popup (Beschreibung beim Überfahren) einzeln ausblenden – Checkboxen `data-tcel`, „↺ Alle anzeigen“ (`data-tcelall`: beim Standard alle sichtbar, bei einer Karte wie im Standard-Design), Speicherort `…default|cards[id].show.{icon,badge,title,fav,open,popup}` (fehlend/true = sichtbar; Admin und Server normalisieren, „Design übertragen“ und „nach EN/DE übertragen“ kopieren es mit). Ausgeblendete Elemente sind unsichtbar und nicht bedienbar, behalten aber ihren Platz – die Karte behält ihre Größe: Seite `getToolCardsCss` → `visibility:hidden; pointer-events:none` auf `.tool-card-icon`/`.tool-card-badge`/`.tool-card-title`/`.tool-card-footer .fav-btn`/`.tool-card-open`, das absolut positionierte Popup `display:none`; ein Einzel-Design setzt nur seine Abweichungen vom Standard (aus- oder mit spezifischerem Selektor wieder einblenden, Popup wieder `display:flex` und auf Touch-Geräten weiterhin aus). Sticky-Vorschau, Popup-Zeile (durchgestrichen), Übersicht „Alle Karten“ und die Ganzseiten-Vorschau im Tab „Layout“ zeigen die Sichtbarkeit identisch. Seite: Variablen `--tc-title-*`, `--tc-badge-*`, `--tc-open-*`, `--tc-desc-*`, `--tc-text-font`, `--tc-align`/`--tc-align-self` auf `.svg-card-link` (damit auch das Popup erbt), `tool-cards.css` liest sie mit Fallbacks; Dunkel-Regeln setzen alle von Hell gesetzten Variablen explizit (Wert oder `initial`), `.tool-card h3.tool-card-title` überstimmt die Basisregel aus components.css (mobil via `--tc-title-size-base`). **Hintergrundbild je Karte** (je Modus; `…light|dark.bgImage/bgImageOpacity/bgImageDarken`): Auswahl aus der Mediathek oder URL, Deckkraft und Abdunkelung; Seite rendert `.tool-card::before` (z-index −1 im isolierten Stacking-Context, `border-radius: inherit`) mit `--tc-img/--tc-img-opacity/--tc-img-filter` (immer explizit, Dunkel ohne Bild = `none`). Gestagte Karten-Bilder laufen über `toolCardImageSlots()`/`allImageSlots()` (Upload in den Sprachordner). Empfehlung: 600 × 400 px (3:2), WebP < 60 KB, Abdunkelung 30–50 %.
  - **Tab „Dateien" mit Modus-Umschalter:** oben eine Sticky-Leiste mit Hell/Dunkel-Umschalter (`data-fileprevmode`, `filesPrevMode` in media.js); darunter die Übersicht „Eingesetzte Dateien“ des gewählten Modus (Dateien des Sprach-Ordners, gemeinsame und Zwischenspeicher, die auf der Seite in diesem Modus erscheinen, mit ihren Plätzen und Zähler „N Dateien, davon X nur dort“), der andere Modus ist eine klickbare Leiste (`.tc-mode-collapsed`, `data-fileshowmode`, Klick/Enter/Leertaste). Die Bild-Plätze aus `siteImageSlots()`/`toolCardImageSlots()` tragen dafür `mode: 'light'|'dark'` (Seiten-Hintergrund, Sektions-Hintergründe, Tool-Karten-Bilder), alle anderen Plätze gelten in beiden Modi (`usageDetailed()` → `{ label, mode }`, `usageOf()` bleibt für die Lösch-Rückfrage). In den Kacheln tragen modusgebundene Plätze ☀️/🌙, Plätze des anderen Modus sind abgeblendet; Kacheln, die im gewählten Modus eingesetzt sind, haben einen Rahmen und die Zeile „☀️ Hell ✓“ bzw. „(nur dieser Modus)“. Neu-Rendern erhält den Scroll (`viewstate.js`).
  - **Tab „Medien" mit Modus-Umschalter:** wie im Layout-Tab ist in der Seitenleiste „Design & Diashow“ nur der Abschnitt des gewählten Modus offen, der andere Modus ist eine schmale, klickbare Leiste (`.tc-mode-collapsed`, `data-smshowmode`, Klick/Enter/Leertaste); Umschalter über der Vorschau (`data-smprevmode`) und Leiste rendern bei einem Wechsel neu (`prevMode` in sectionmedia.js, Sichtzustand bleibt), „→ Dunkel/Hell kopieren“, „★ Für alle Sektionen“, „↺“ und „↺ Alles“ öffnen den betroffenen Modus; die rechte Seitenleiste (Text) bleibt modusunabhängig.
  - **Hero-Hintergrundbild und Buttons ein-/ausblenden (ausgeblendete Buttons bleiben im Markup und werden per `.hero-hidden` mit `visibility: hidden` + `pointer-events: none` unsichtbar, `aria-hidden`, CTA `tabindex=-1` – der Platz bleibt erhalten, der Hero behält seine Höhe; die Admin-Vorschau blendet ebenso per `visibility` aus)** (Tab „Hero-Design"): In den Seitenleisten je Modus die Sektion „🏞️ Hintergrundbild“ – Bild aus der Mediathek oder per URL (`/uploads/…` oder `https://…`; gestagte Bilder bis zum Veröffentlichen als `staged:<id>`), Regler Deckkraft (0–100 %), Abdunkelung (0–100 %), Weichzeichner (0–20 px), Sättigung (0–200 %), „Entfernen“ und „↺“ (Bild + Regler); Speicherort `media.<lang>.heroDesign.light|dark.bgImage/bgImageOpacity/bgImageDarken/bgImageBlur/bgImageSaturate` (`HERO_IMG_FIELDS` in model-hero.js, Server `validateHeroSide`). Die Bilder laufen über `heroImageSlots()` in `allImageSlots()` (Upload beim Veröffentlichen, „wird verwendet in“, Modus-Übersicht im Dateien-Tab). Wirkt unabhängig vom An-Schalter „Eigenes Hero-Design“; „Farben nach Hell/Dunkel kopieren“ lässt das Bild des Ziel-Modus unangetastet. In der Mitte die Sektion „👁️ Texte & Buttons ein-/ausblenden“ mit `heroDesign.showTitle` (Titel), `heroDesign.showSubtitle` (Untertitel), `heroDesign.showChips` (alle Feature-Chips), darunter je Feature-Button ein eigener Schalter (`heroDesign.hiddenChips[]` = Schlüssel der ausgeblendeten Buttons, `normHiddenChips` in model-hero.js, Server-Prüfung in `validateHeroDesign`; bei ausgeschaltetem Gesamtschalter deaktiviert) und `heroDesign.showCta` (CTA „Jetzt starten“), beide Modi; die Sticky-Vorschau zeigt **alle** Feature-Buttons (6 Spalten), die Bildebene und ausgeblendete Buttons live (einzeln per `visibility`, der Platz bleibt). **Klick (oder Enter/Leertaste) auf einen Text oder Button in der Vorschau** (`data-hdtitle`, `data-hdsub`, `data-hdchip`, `data-hdcta`, `hdSelected`/`applyHdSelection` in design.js) klappt in der offenen Seitenleiste die passende Sektion auf (`data-hdsection`: „Hero-Texte – Farbe je Text“ mit markiertem Text-Block `data-hdtxtside`, „Buttons (Feature-Chips)“, „CTA-Button“), scrollt nur die Seitenleiste dorthin und hebt sie hervor; in der Mitte werden Schalterzeile (`data-hdshowrow`), Beschriftungszeile (`data-hdfeatrow`) bzw. die Textzeile in „Hero-Texte“ (`data-hdtxtrow`, Sektion wird aufgeklappt) markiert. Die Markierung bleibt bis zur nächsten Auswahl erhalten (auch bei Live-Aktualisierung und Modus-Wechsel). Seite: `.hero-title`/`.hero-subtitle` bei `showTitle`/`showSubtitle` = false sowie `feature-card` mit Schlüssel in `hiddenChips` bekommen `.hero-hidden` + `aria-hidden` (Platz bleibt). **Hero-Texte verschieben:** Titel, Untertitel und CTA lassen sich einzeln versetzen – in der Vorschau mit der Maus ziehen (Pointer-Events, 1:1 dank Originalgröße; ein Zug ist kein Klick/keine Auswahl), mit den Pfeiltasten auf dem fokussierten Text (1 px, Shift = 10 px) oder über die Felder „Verschiebung X / Y (px)“ in der Feinabstimmung je Text (`data-txtoff`, ↺ `data-txtreset="<key>:off"`). Speicherort `media.<lang>.textStyles["hero.*"].offsetX/offsetY` (±400/±300 px, nur bei ≠ 0 gespeichert; `TEXT_OFFSET_MAX` in model-hero.js, Server `validateTextStyles`), „Hero-Design nach EN/DE übertragen“ nimmt sie mit. Seite (`getTextStylesCss`) und Vorschau setzen `position:relative; left/top` – kein `transform`, weil die Entrance-Animationen (`fadeInUp`, fill-mode forwards) transform überschreiben würden; der Platz im Fluss bleibt, der Hero behält seine Höhe. **Feature-Buttons verschieben:** genauso je Button (Ziehen in der Vorschau, Pfeiltasten, Felder „Verschiebung X / Y“ unter der Beschriftung in „Buttons & CTA“, `data-chipoff`/`data-chipoffreset`); Speicherort `heroDesign.chipOffsets[key] = { x, y }` (nur ≠ 0, max. 20 Einträge; `normChipOffsets` in model-hero.js, Server `validateHeroDesign`), Seite setzt `left/top` inline auf `.feature-card` (ist bereits `position:relative`; der Hover-/Tilt-Effekt nutzt `transform` und bleibt unberührt). Ein gemeinsamer Mechanismus (`bindMovable` mit `textMover`/`chipMover` in design.js) bedient Texte und Buttons. Seite: `content.ts` → `getHeroImageCss` erzeugt `.hero{--hero-img…}` und eine Dunkel-Regel (setzt ohne Bild explizit `none`), `hero.css` rendert die Ebene als `.hero::before` (z-index 0 unter `.hero-content`, `border-radius: inherit`, negativer Inset bei Weichzeichner); beide Startseiten rendern `.features-grid` bzw. `.hero-cta-wrap` nur bei `showChips`/`showCta` (die Tilt-Scripts prüfen ohnehin auf Vorhandensein).
  - **Hero-Hintergrund als Diashow** (Tab „Hero-Design", Seitenleisten, je Modus): unter dem Hintergrundbild der klappbare Bereich „🎞️ Diashow – weitere Bilder“ (bis zu 12 weitere Bilder aus der Mediathek, ↑/↓, ✕, „Alle entfernen“; Takt, Übergangsdauer, Übergang, Pause bei Mauszeiger, Punkte – Punkte standardmäßig aus). Speicherort `heroDesign.light|dark.bgSlides[]` und `bgSlideshow` (Admin `normHeroSide`, Server `validateHeroSide` → `validateSlideList`/`validateBannerSlideshow`; die Bilder laufen über `heroImageSlots()` beim Veröffentlichen und in der Nutzungsanzeige). Die Sticky-Vorschau wechselt das Bild der Bildebene im Takt. Beide Modi haben ihre eigene Diashow (Bilder und Takt getrennt); „Bild & Diashow nach Dunkel/Hell kopieren“ (`data-hdimgcopy`) übernimmt Bild, Bearbeitung und Diashow in den anderen Modus und öffnet ihn. Seite: `HeroBackground.astro` rendert je Modus die Ebene `.hero-bg--light|dark` (erste Kinder von `.hero`, `pointer-events: none`, Dunkel-Ebene nur im Dunkelmodus sichtbar) – ein Bild oder `HeroSlides` (Variante „cell“, Hover-Pause über `pauseTarget=".hero"`, da die Ebene selbst keine Mausereignisse annimmt); `content.ts` → `heroBgImages()` und `getHeroImageCss()` (Deckkraft/Filter/Inset direkt auf `.hero-bg--<modus>`). Das frühere `.hero::before` ist damit ersetzt.
  - **Datenmodell `model.js` in Teilmodule aufgeteilt** (ohne Verhaltensänderung; gerendertes HTML aller zehn Tabs in DE + EN byteidentisch): `model.js` ist nur noch der Bündler (`export *` aus allen Teilmodulen, alle Importe der Tabs bleiben bei `./model.js`) und füllt nach dem Laden `state.ticker`, `state.media` und `state.loadedMedia`; `model-state.js` hält den App-Zustand ohne eigene Importe (damit können alle Teilmodule `state` nutzen, ohne einen Import-Kreis zu bilden); `model-core.js` (Farb-/Schrift-/URL-Helfer, `rgbaFromHex`, `MEDIA_LANGS`/`MEDIA_KEYS`, `BANNER_ANIM_*`, Pfad-Helfer, `SUBTABS`); `model-ticker.js`; `model-hero.js` (Hero-Design + Text-Stile); `model-toolcards.js` (Karten-Design, Icon-Färbung, Karten-Bild-Plätze); `model-grid.js` (Raster, Kachel-/Banner-Design, Diashows); `model-site.js` (Seiten-Hintergrund, Sektionen, Effekte, ausgeblendete Karten, globale Schrift, Seiten-Bild-Plätze); `model-sectionmedia.js`; `model-media.js` (Medien-Standard je Sprache, `normalizeMedia`, Slot-Zugriff, `allImageSlots`). `normIconTint` und `normHeroLayout` sind dafür exportiert.
  - **Layout-Tab in Module aufgeteilt** (ohne Verhaltensänderung, gerendertes HTML byteidentisch): `layout.js` (Orchestrierung: Modus-Panel Banner/Raster, `captureView`/`restoreView`, Undo/Redo-Sync, ruft `bindBanner`/`bindGrid` mit `renderLayout` als Rückruf; re-exportiert die Helfer für andere Tabs), `layout-banner.js` (Banner-Design Hell/Dunkel mit `bannerPrevMode`, Banner-Text, Banner-Medium, Diashow, Live-Vorschau), `layout-grid.js` (Kachel-Vorschau, Inhalts-/Design-Editoren, Kachel-Diashow, `activeCell`/`markCell`), `layout-paste.js` (Zwischenablage → Kachel/Banner/Diashow; bekommt Neu-Rendern und Kachel-Markierung über `initPaste()` aus layout.js, kein Import-Kreis), `layout-shared.js` (`clamp`, `fontFF`, `slideInfo`, `slideshowSettingsHtml`, `overlayStyle`, `dragHandle`, Reset-Buttons, Timer der Vorschau-Diashow). Alle IDs, `data-`-Attribute und CSS-Klassen unverändert.
  - **Tab „Layout" (Banner) mit Modus-Umschalter:** Der Umschalter über der Sticky-Vorschau (`data-bannerprevmode`) wählt den bearbeiteten Modus; in der Seitenleiste „Banner-Design“ ist nur dessen Abschnitt offen, der andere Modus ist zu einer schmalen, klickbaren Leiste zugeklappt (`.tc-mode-collapsed`, `data-bannershowmode`, Klick/Enter/Leertaste), da die rechte Seitenleiste (Banner-Text) modusunabhängig bleibt. Ein Wechsel rendert neu mit erhaltenem Sichtzustand (`bannerPrevMode` in layout.js); „→ Dunkel/Hell kopieren“, „↺“ und „↺ Alles“ öffnen den betroffenen Modus. Das Kachel-Raster hat kein Hell/Dunkel-Design und bleibt unverändert.
  - **Tab „Texte" mit Modus-Umschalter:** oben in der Mitte eine Sticky-Leiste mit dem Umschalter (`data-txtprevmode`); die Seitenleiste mit Vorschau und Textfarben des gewählten Modus ist offen, die des anderen zugeklappt (`data-txtshowmode`, `txtPrevMode` in content.js).
  - **Tab „Tool-Karten" mit Modus-Umschalter:** Sticky-Vorschau zeigt nur den gewählten Modus (`data-tcprevmode`, `.tc-previews--single`), die Seitenleiste des anderen Modus ist zugeklappt (`data-tcshowmode`, `tcPrevMode` in toolcards.js); die Übersicht „Alle Karten“ folgt dem gewählten Modus (`ovTheme`).
  - **Tab „Hero-Design" mit Modus-Umschalter:** wie im Hintergrund-Tab zeigt die Sticky-Vorschau nur den gewählten Modus in voller Breite (`.tc-previews--single`, Umschalter `data-hdprevmode`), die Seitenleiste des anderen Modus ist zugeklappt (`.tc-side--collapsed`, `data-hdshowmode`, `hdPrevMode` in design.js); Hover-Regeln und Live-Aktualisierung greifen nur auf die vorhandene Vorschau. **Vorschau in Originalgröße:** der Hero-Kasten wird mit den Maßen der Seite gerendert (1200 px breit, Innenabstand 4rem 2rem, Radius 2rem, Grundschrift 16 px/Zeilenhöhe 1.6; Titel 2.5rem, Untertitel 1.1rem/max. 600 px, Chips 0.95rem mit Seitenabständen, CTA 1.05rem – Standardwerte, falls im Admin keine Größe gesetzt ist) und liegt in einem Scrollrahmen fester Höhe (`data-hdscroll`, 300 px, `overflow:auto`, `resize:vertical` – der Admin kann ihn unten rechts höher ziehen; `hdPrevHeight` in design.js merkt die gezogene Höhe, `data-tcside="hdprev"` erhält Scroll-Position senkrecht und waagerecht über ein Neu-Rendern via `viewstate.js`).
  - **Tab „Hintergrund" mit Modus-Umschalter:** Die Sticky-Vorschau zeigt nur den gewählten Modus (Hell oder Dunkel) in voller Breite (`.fx-prevs--single`, Umschalter `.mode-switch`); die Seitenleiste des anderen Modus ist zu einer schmalen, klickbaren Leiste zugeklappt (`.tc-side--collapsed`, `data-bgshowmode`; Raster `.tc-layout.bg-collapsed-<modus>` = 64 px / bis 1200 px / bis 420 px), Klick darauf oder auf den Umschalter wechselt Vorschau und bearbeitbare Seite (`bgPrevMode` in background.js, Neu-Rendern mit erhaltenem Sichtzustand). Unter 1500 px wird die zugeklappte Leiste zu einem horizontalen Balken.
  - **Tab „Medien" dreispaltig** (`sectionmedia.js`, `.tc-layout`/`.app-wide`): Mitte Sektionswahl (Audio-/Bild-/Diverse-Tools, Marker ✍️/🎞️ bei Text/Diashow), Sticky-16:9-Vorschau mit Hell/Dunkel-Umschalter (`.mode-switch`, aktiver Modus farbig; Hintergrund = Seitenhintergrund des Modus aus dem Tab „Hintergrund“ bzw. Seitenstandard, Etikett „Hell-/Dunkel-Ansicht“; springt beim Bearbeiten in den Modus des Feldes, auch per Klick auf den Abschnittskopf ☀️/🌙 in der Seitenleiste; der Banner-Umschalter im Layout-Tab nutzt denselben Stil), Medium-Slot (Video **oder** Bild: Pfad/URL, „Aus Zwischenspeicher wählen“, „Standard“; Prüfung auf fehlende `/uploads`-Datei) und darunter unverändert der Hero-Bereich. **Links „Design & Diashow“** je Hell/Dunkel: „Eigener Umriss“ (aus = Standard-Rahmen aus components.css; an = Farbe + Dicke, 0 = kein Rahmen), Eckenradius, Deckkraft, Verdunkelung, „→ Dunkel/Hell kopieren“, „★ Für alle Sektionen“ (kopiert das Design dieses Modus mit Rückfrage in die beiden anderen Sektionen derselben Sprache; der andere Modus bleibt), „↺ Alles“; Diashow nur wenn das Medium ein Bild ist (bei Video Hinweis, Bilder bleiben gespeichert): weitere Bilder (Zwischenspeicher, ↑/↓, ✕), Takt, Übergangsdauer, Übergang, Pause, Punkte; Vorschau wechselt die Bilder. **Rechts „Text“**: Text (max. 200 Zeichen), Schriftart aus `/fonts`, Farbe, Größe, Position (⤒ ◎ ⤓ oder Ziehen in der Vorschau), Deckkraft, Schatten (an/aus, Farbe, Versatz, Weichzeichnung), Umriss (Farbe, Dicke). „Sektions-Design nach EN/DE übertragen“ kopiert Design (Hell + Dunkel), Diashow-Einstellungen und Text-Design – Medium, Bilder und Text bleiben je Sprache. Speicherort `media.<lang>.sectionMedia.{audio|image|diverse}` = `{ style:{light,dark}, slides[], slideshow, text }` (Admin `normSectionMedia*`, Server `validateSectionMediaAll`; Diashow-Bilder über `sectionMediaImageSlots()` in `allImageSlots()`). Seite: `SectionMediaBlock.astro` ersetzt `<SectionMedia>` in den drei Sektionen beider Startseiten – gleicher Wrapper `.section-video-wrapper` (+ `.section-media-wrapper[data-section-media]`), Medium in `.section-media-inner` bzw. bei Diashow in `HeroSlides` (Variante `cell`), Text als `.section-media-text` (Inline-Style; `textFontId` aus content.ts); `getSectionMediaCss` erzeugt nur Abweichungen (Rahmen/Radius am Wrapper, Deckkraft/Filter am Medium, Dunkel-Regeln setzen Hell-Werte zurück) plus `@font-face` der Text-Schrift.
  - **Tab „Layout" dreispaltig** wie die anderen Design-Tabs (`.tc-layout`, `.app-wide` via `renderMain`): im Raster-Modus Seitenleiste **Kachel-Inhalte** links (Bild, Bildbearbeitung, Text, Schriftart) und **Kachel-Design** rechts (Rahmen, Hintergrund, Textfarbe/-größe/-position, „Standard für alle"), in der Mitte Modus, Sticky-Live-Vorschau, Anordnung und Form; Klick auf eine Vorschau-Kachel hebt beide Editoren hervor und scrollt nur die Seitenleisten dorthin (Seite bleibt stehen); Neu-Rendern erhält Scroll-Positionen (`viewstate.js`). „Kachel-Design nach EN/DE übertragen“ (Seitenleiste Kachel-Design) kopiert `heroLayout`, `heroGridRatio`, `heroGridFit`, `heroGridUniform(Cell)` und je Kachel alle `heroGridStyles`-Felder außer `text` in die andere Sprache; Kachel-Texte, Bilder (`heroGrid`) und Links bleiben je Sprache. Im Banner-Modus steht in der Mitte unter der Vorschau das Panel **Banner (Bild oder Video)** mit allem, was vorher im Medien-Tab lag: Vorschaubild, Status (lokal / veröffentlicht, Warnung bei fehlender `/uploads`-Datei per HEAD-Prüfung), Pfad/URL, „Aus Zwischenablage einfügen“ (Clipboard-API; Strg/Cmd+V im Banner-Modus setzt ebenfalls das Banner), „Aus Mediathek“ (`openMediaPicker` mit `onPick`, bleibt im Tab), „Entfernen“ und die Verlinkung (`heroBannerLink`); der Medien-Tab zeigt im Banner-Modus nur noch Vorschau + Button „Zum Tab Layout“. **Diashow:** Panel „Diashow – weitere Bilder“ unter dem Banner-Panel: bis zu 12 weitere Bilder (nur Bilder) aus Zwischenablage oder Mediathek (`imagesOnly`), Reihenfolge per ↑/↓, Entfernen einzeln oder alle; Einstellungen Anzeigedauer (1–30 s), Übergangsdauer (0–5000 ms, CSS-Variable `--slide-fade`), Übergang (`fade` Überblenden, `slide` Schieben, `zoom` Ken-Burns, `none` Schnitt), Pause bei Mauszeiger, Punkte. Speicherort `media.<lang>.heroBannerSlides` (Array von URLs bzw. `staged:<id>` bis zum Veröffentlichen; `heroSlideImageSlots()` hängt sie an `allImageSlots()`, damit Upload/Ersetzen/Nutzungsanzeige wie bei allen anderen Bild-Plätzen laufen) und `heroBannerSlideshow`; Server validiert URLs/Bereiche. Seite: `src/components/HeroBannerMedia.astro` ersetzt das nackte `<SectionMedia>` im Banner beider Startseiten – ohne weitere Bilder identische Ausgabe, sonst `.hero-slides` (Container in Größe des ersten Bildes, weitere Bilder absolut mit `object-fit: cover`, Punkte als `role="button"`, Script wechselt per `is-active`/`is-prev`, pausiert bei Hover und im Hintergrund-Tab); das Banner-Design (`getHeroBannerCss`) wirkt bei einer Diashow auf den Container, die Einzelbilder setzt hero.css neutral; `prefers-reduced-motion` schaltet Übergänge ab. **Diashow im Kachel-Raster:** je Kachel eine eigene Liste weiterer Bilder (`media.<lang>.heroGridSlides[i]`, Seitenleiste „Kachel-Inhalte“ → aufklappbar „Diashow – weitere Bilder“ mit Anhängen aus Zwischenablage/Mediathek, ↑/↓, ✕) und gemeinsame Einstellungen aller Kacheln in `heroGridSlideshow` (Panel „Diashow der Kacheln“ in der Mitte; zusätzlich `stagger` = Kacheln wechseln versetzt, gleichmäßig über das Intervall verteilt); die Sticky-Vorschau wechselt die Kachelbilder im Takt. Seite: `HeroCellMedia.astro` nutzt den gemeinsamen Container `HeroSlides.astro` (Variante `cell`: füllt die Kachel, jedes Bild im `.hero-cell-media`-Wrapper, damit Deckkraft/Abdunkelung/Weichzeichner für alle gelten; Punkte kleiner; `data-offset` für den versetzten Start). „Kachel-Design übertragen“ kopiert die Diashow-Einstellungen (nicht die Bilder), „Banner-Design übertragen“ ebenso `heroBannerSlideshow`. Links **Banner-Design** getrennt für **Hell** und **Dunkel** (je Modus Rahmenfarbe/-dicke, Eckenradius, Schatten an/aus mit Farbe/Versatz/Weichzeichnung/Deckkraft, Deckkraft und Verdunkelung des Banner-Bildes/-Videos; „↺“ je Feld, „↺ Alles“ je Modus, „→ Dunkel/Hell kopieren“; die Live-Vorschau hat einen Hell/Dunkel-Umschalter mit passendem Seitenhintergrund und springt beim Bearbeiten automatisch in den Modus des Feldes; Felder tragen `data-mode`, Regler-IDs `ly:banner:<mode>:<feld>`). **Übertragen zwischen Sprachen:** „Banner-Design nach EN/DE übertragen“ kopiert `heroBannerStyle` (Hell + Dunkel) in die andere Sprache, „Text-Design nach EN/DE übertragen“ kopiert Schrift, Farbe, Größe, Position und Effekte des Banner-Textes – der Text selbst (`heroBannerText`) bleibt je Sprache. Im Tab **Hero-Design** überträgt „Hero-Design nach EN/DE übertragen“ (Mitte, oben) `heroDesign` (Hell + Dunkel) und `textStyles["hero.*"]` von der aktuellen in die andere Sprache; Hero-Texte und Button-Beschriftungen (Overrides) bleiben je Sprache. Im Tab **Tool-Karten** überträgt „Tool-Karten-Design nach EN/DE übertragen“ (Kopf des Tabs) `toolCards` (An-Schalter, Standard + Einzel-Designs, Hell + Dunkel, Typografie, Sichtbarkeit der Elemente) von der aktuellen in die andere Sprache; Karten-Texte (Overrides) bleiben je Sprache. Beide Buttons ersetzen die frühere Einbahn-Kopie Deutsch → Englisch. Speicherort `media.<lang>.heroBannerStyle.light|dark` (Alt-Format flach = beide Modi; Admin und Server migrieren), Server validiert Hex/Bereiche; Seite: `content.ts` → `getHeroBannerCss` erzeugt nur bei Abweichung vom Standard eine Hell-Regel auf `.hero-banner-wrapper .hero-banner` und eine Dunkel-Regel unter `[data-theme="dark"]`, die alle von Hell gesetzten Eigenschaften explizit setzt oder auf den Basiswert zurücknimmt (Standard = bisheriges Aussehen aus hero.css). Rechts **Banner-Text** (Text, Schrift, Farbe, Größe, Position sowie Text-Effekte: Schatten, Umriss, Deckkraft, Animation). Unter 1500 px stapeln sich die Spalten (Mitte, dann links, dann rechts).
  - **Tab „Layout": Ganzseiten-Vorschau mit verschiebbaren Medien und Tool-Karten** (`layout-fullpage.js`: `fullPageHtml`/`bindFullPage`, von `layout.js` unten in `.tc-main` eingehängt – in Banner- und Raster-Modus): aufklappbarer Bereich „🗺️ Ganze Seite – Vorschau: Medien & Tool-Karten verschieben“ mit eigenem Hell/Dunkel-Umschalter (`data-smfullmode`) und einer schematischen Startseite (Hero mit Hero-Medium – Banner oder Kachel-Raster inkl. ausgeblendeter Kacheln –, Titel/Untertitel/Buttons als Platzhalter; die drei Tool-Sektionen mit Überschrift, Medium samt Design/Text-Overlay und den **echten Tool-Karten** in den Maßen der Seite – Raster wie auf der Seite – Audio-Tools `.svg-card-grid` ab 1024 px (4 gleiche Spalten), Bild-/Diverse-Tools zusätzlich `.grid-centered` (`repeat(auto-fit, minmax(210px, 300px))`, zentriert – bei drei Karten also 300 px breit und mittig), 1.25rem Abstand, gleich hohe Reihen (`data-smfullgrid`), Karte wie `.tool-card` (Innenabstand 1.1rem/0.85rem, Icon 44 px – ein leerer svg-Override entfernt es wie auf der Seite –, Badge, Titel mit 2 reservierten Zeilen und 0.35rem Abstand, Fußzeile mit 28-px-Favoritenknopf), Rahmen/Hintergrund/Typografie aus dem Tab „Tool-Karten“ (`getToolCards`: Standard oder Einzel-Design, sonst Seitenstandard), ausgeblendete Karten entfallen; die Seitenschrift „Supreme“ wird mit denselben Schnitten (400/500/700/800 aus `/fonts`) eingebettet, sofern keine globale Schrift gesetzt ist – Zeilenumbrüche und Kartenhöhen entsprechen so der Seite (gemessen: 269 × 183 px in der Vorschau vs. 269 × 184 px online), damit der gezogene Versatz 1:1 stimmt). Die Seite wird in Originalbreite (1200 px) aufgebaut und per CSS `zoom` in die Spalte eingepasst (`data-scale`, ResizeObserver; Rahmen `data-smfullwrap` scrollbar, max. 70 vh, in der Höhe ziehbar). **Hero-Medium** (`data-smfullmedia="hero"`), **Sektions-Medien** (`audio|image|diverse`) und **jede Tool-Karte** (`card:<sektion.key>`) lassen sich mit der Maus ziehen (Pointer-Events, Delta ÷ Zoom = px auf der Seite; ein Zug ist kein Klick, Klick markiert), mit Pfeiltasten (1 px, Shift 10 px) bewegen oder über das **Auswahl-Panel** „🎯 Ausgewählt“ exakt setzen: Dropdown aller Elemente (`data-smselkey`: Hero-Medium, Sektions-Medien, Tool-Karten je Sektion; Auswahl folgt Klick, Zug und Pfeiltasten in der Vorschau, das Dropdown scrollt das Element im Rahmen in Sicht), **absolute Spinner „Links/Oben“** (`data-smsell`/`data-smselt`: linke/obere Kante in Seiten-px, gemessen am Rand der 1200-px-Inhaltsspalte bzw. am oberen Seitenrand – gleiche Werte = bündig; Eingabe rechnet in den Versatz um: Versatz = Kante − Ausgangsplatz, Ausgangsplatz aus `originOf` = Elementposition ohne Versatz ÷ Zoom), daneben die **Versatz-Spinner X/Y** (`data-smselx`/`data-smsely`, Abstand zum eigenen Ausgangsplatz – das ist der gespeicherte Wert; zwei gleich weit verschobene Elemente zeigen denselben Versatz), alle mit 1-px-Schritten und Live-Abgleich beim Ziehen, sowie ↺ (`data-smselreset`, setzt Versatz und Größe zurück). **Tool-Karten skalieren:** die markierte Karte zeigt sechs Skalierungspunkte (`data-smhandle`: Ecken nw/ne/sw/se = proportional, die stärkere Mausbewegung bestimmt den Faktor; Kanten e/s = nur Breite bzw. Höhe; bei west/nord bleibt die gegenüberliegende Kante stehen, der Versatz wandert mit), dazu Spinner „B/H“ in % (`data-smselw`/`data-smselh`, 25–300) und daneben die skalierte Größe in **Pixeln** der Seite (`data-smselwpx`/`data-smselhpx`; Grundmaß = Elementgröße ÷ Zoom ÷ Faktor, Eingabe rechnet in Prozent um, beide Felder bleiben synchron). Speicherort `toolCardOffsets[key].w/h` (nur ≠ 100), Seite und Vorschau rendern `transform: scale(w/100, h/100); transform-origin: top left` auf `.svg-card-link` – der Platz im Raster bleibt, Icon/Badge/Titel skalieren mit; Rückgängig und „Ursprüngliche Anordnung“ umfassen die Größe; **Texte:** Hero-Titel, Hero-Untertitel, Hero-Button und die drei Abschnitts-Überschriften sind in der Ganzseiten-Vorschau ebenfalls verschiebbar (`data-smfullmedia="text:<slot>"`, Gruppe „Texte“ im Dropdown, gleiche Aktionen: Ziehen, Pfeiltasten, Spinner, Rückgängig, Ursprüngliche Anordnung) – Speicherort `media.<lang>.textStyles[slot].offsetX/offsetY`, also derselbe wie im Tab „Hero-Design“ (Hero-Texte) und wie die Seite ihn für alle Text-Slots rendert (`getTextStylesCss`: `position:relative; left/top`); bei „Standard für alle Slots“ (Texte-Tab) teilen sich die drei Überschriften den Versatz des Master-Slots und bewegen sich gemeinsam. Grenzen der Text-Verschiebung ebenfalls ±1200 px (`TEXT_OFFSET_MAX`, Server, Seite); verschobene Karten stehen mit Versatz und ↺ in einer Liste (`data-smcardsinfo`, „↺ alle Karten“ `data-smcardsreset`). **Verlauf:** jede Änderung (Zug, Pfeiltasten/Felder desselben Elements innerhalb 800 ms zusammengefasst, ↺) merkt den Zustand davor je Sprache (`histBy`, max. 200); „↶ Rückgängig (n)“ (`data-smundo`) stellt den letzten Schritt wieder her, auch nach einem Neu-Zeichnen; „↺ Ursprüngliche Anordnung“ (`data-smresetall`) setzt alle Versätze (Hero-Medium, Sektions-Medien, Tool-Karten) auf 0 = ursprüngliche Seitenanordnung und ist per Rückgängig zurückholbar. **Hilfsraster (temporär, nur Vorschau):** Schalter „📐 Hilfsraster“ (`data-smgrid`), Rasterweite 10/20/25/50/100 px (`data-smgridstep`, jede fünfte Linie kräftiger, Ebene `data-smgridoverlay` über der Seite, nicht bedienbar) und „einrasten“ (`data-smsnap`): beim Ziehen rastet die linke/obere Kante des Elements auf Rasterlinien ein (absolute Position in Seiten-px, Versatz = Linie − Ausgangsposition), Pfeiltasten springen zur nächsten Linie (Shift: fünf Linien); ohne Raster wie bisher 1 px/10 px. Live-Anzeige `data-smlive` zeigt Element und Versatz. Bewegungsfreiheit: Grenzen ±1200 px (`MEDIA_OFFSET_MAX`, Server-Clamps), damit z. B. eine Karte aus der letzten Reihe/Spalte überall auf der Seite landen kann; verschobene Elemente liegen mit `z-index:2` über unverschobenen (Vorschau und Seite; Hover der Karte bleibt darüber). Speicherort: `media.<lang>.sectionMedia.<key>.offsetX/offsetY`, `media.<lang>.heroMediaOffsetX/Y` und `media.<lang>.toolCardOffsets['<sektion.key>'] = { x, y, w?, h? }` (±1200 px, w/h in % 25–300, Karten nur ≠ 0 bzw. ≠ 100, max. 60; `normMediaOffset`/`MEDIA_OFFSET_MAX` in model-sectionmedia.js, `normToolCardOffsets` in model-media.js, Server `validateSectionMediaAll`/`validateMediaLocale`). Seite: `SectionMediaBlock.astro` setzt `left/top` inline auf `.section-media-wrapper`, die Startseiten auf `.hero-promo` und je Karte auf `.svg-card-link` (alle `position:relative`) – der Platz im Fluss bleibt, nur das Element wandert. `sectionmedia.js` exportiert dafür `mediaInfo`/`designCss`/`textStyle`/`MEDIA_BASE` und bezieht seine Helfer direkt aus `layout-shared.js` (kein Import-Zyklus).
  - **Einzelbanner und Kacheln ein-/ausblenden (Tab „Layout")**: Im Banner-Modus schaltet „Banner anzeigen“ über der Vorschau (`data-bannershow`) das Banner ab – `media.<lang>.heroBannerShow=false`; die Seite rendert dann weder Banner noch Banner-Text (Bild, Text, Design bleiben gespeichert), die Admin-Vorschau zeigt es abgeblendet mit Hinweis. Im Raster-Modus hat jeder Kachel-Editor (links) einen Schalter „anzeigen“ (`data-cellshow`) – `media.<lang>.heroGridHidden[i]=true`; ausgeblendete Kacheln entfallen im Raster, die **übrigen teilen den Platz symmetrisch**: das Raster wird zum zentrierten Flex-Umbruch (`.hero-grid--reflow`, `--hero-cols` = Kacheln je Reihe aus `heroGridReflowCols(layout, n)`: so viele Reihen wie nötig, gleichmäßig verteilt – 3×2 mit 5 sichtbaren → 3 + 2 zentriert, mit 4 → 2×2; Mosaik ohne große Kachel → gleich große nebeneinander), Kachelform (`--hero-cell-ratio`) bleibt. **Einblenden** (Schalter oder Knopf „Kachel n einblenden“ im Hinweis über der Vorschau, `data-cellshowbtn`) entfernt die Klasse – die ursprüngliche Anordnung ist wieder da. Gleiche Formel im Admin (`model-grid.js`: `HERO_LAYOUT_COLS`, `heroGridReflowCols`, `visibleGridCells`) und auf der Seite (`content.ts`); Server prüft beide Felder in `validateMediaLocale`. Ausgeblendete Kacheln außerhalb des aktuellen Layouts (z. B. Kachel 5 bei „4 nebeneinander“) bleiben gemerkt und tauchen beim Wechsel zu einem größeren Layout wieder im Hinweis auf.
  ...
  - **Tab „Icons" mit Modus-Umschalter:** über der Liste „Tool-Karten-Icons verwalten“ wählt ein Hell/Dunkel-Umschalter (`data-tintprevmode`, `tintMode` in icons.js) den bearbeiteten Modus für alle Karten: die Vorschau in der Liste zeigt das Icon mit Färbung und Kasten dieses Modus, ein geöffneter Farbbereich („🎨 Farben“) enthält nur Vorschau, Icon-Farbe und Kasten dieses Modus, der andere Modus ist eine klickbare Leiste (`.tc-mode-collapsed`, `data-tintshowmode`, Klick/Enter/Leertaste); nur die Liste wird neu gerendert, Bibliothek und Auswahl bleiben.
