...
- `server/admin/public/` — eigenständige Vanilla-JS-App (kein Build-Schritt), vom Node-Dienst ausgeliefert:
  ...
  - **Tab „Medien": Ganzseiten-Vorschau mit verschiebbaren Medien** (`fullPageHtml`/`bindFullPage` in `sectionmedia.js`, eingebunden in `media.js` unter dem Medium-Slot): aufklappbarer Bereich „🗺️ Ganze Seite – Vorschau: Medien verschieben“ mit einer schematischen Startseite (Hero mit Hero-Medium – Banner oder Kachel-Raster inkl. ausgeblendeter Kacheln –, Titel/Untertitel/Buttons als Platzhalter; die drei Tool-Sektionen mit Überschrift, Medium samt Design/Text-Overlay und Karten-Platzhaltern), im Hell-/Dunkelmodus des Umschalters über der Sticky-Vorschau. Die Seite wird in Originalbreite (1200 px) aufgebaut und per CSS `zoom` in die Spalte eingepasst (`data-scale`, ResizeObserver; Rahmen `data-smfullwrap` scrollbar, max. 70 vh, in der Höhe ziehbar). **Hero-Medium** (`data-smfullmedia="hero"`) und **Sektions-Medien** (`audio|image|diverse`) lassen sich mit der Maus ziehen (Pointer-Events, Delta ÷ Zoom = px auf der Seite; ein Zug ist kein Klick), mit Pfeiltasten (1 px, Shift 10 px) bewegen oder über Felder „X / Y“ je Medium setzen (`data-smoff`, ↺ `data-smoffreset`); Klick auf ein Sektions-Medium wählt die Sektion (Seitenleisten wechseln). Speicherort: `media.<lang>.sectionMedia.<key>.offsetX/offsetY` und `media.<lang>.heroMediaOffsetX/Y` (±400/±300 px; `normMediaOffset`/`MEDIA_OFFSET_MAX` in model-sectionmedia.js, Server `validateSectionMediaAll`/`validateMediaLocale`). Seite: `SectionMediaBlock.astro` setzt `left/top` inline auf `.section-media-wrapper` (bereits `position:relative`), die Startseiten auf `.hero-promo` (`position:relative`) – der Platz im Fluss bleibt, nur das Medium wandert.
